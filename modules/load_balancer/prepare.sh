...
wget --output-document="/tmp/p.tar.gz" ${download_url}
tar xf /tmp/p.tar.gz -C $APP_PATH
ln -s $APP_PATH/${package_base_dir}/${app_dir} $APP_PATH/www
chown $USER:$GROUP $APP_PATH/www
chown -R $USER:$GROUP $APP_PATH/${package_base_dir}/${app_dir}
pip3 install -r $APP_PATH/${package_base_dir}/requirements.txt
